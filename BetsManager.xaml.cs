		...
		private void ApplyFilter()
		{
            using (Rows.SuspendNotifications())
            {
                ...
                foreach (var row in _allRows)
                {
                    ...
                    {
						Debug.WriteLine( $"[FILTER] BetId={row.BetID} " + $"IsMatched={row.IsMatched} " + $"UnmatchedOnly={UnmatchedOnly}");
						Rows.Add(row);
                    }
                }
                        ...
                        if (MarketNode != null)
                        {
                            await Task.Run(() =>
                            {
                                ...
                                foreach (BetsManagerRow row in Rows)
                                {
                                    ...
                                    {
										cancel_instructions.Add(new CancelInstruction(row.BetID) { sizeReduction = Math.Round((row.Stake), 2) });
                                        row.Stake /= 2;
										_simulatedStream?.SimulateCancel(row.BetID.ToString());
									}
								}
